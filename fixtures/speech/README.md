...
`align-tts-mpl-fixture/` is the deliberate small exception: Coqui did not
publish a pretrained Align-TTS entry in its model registry, so the repository
commits a deterministic tiny checkpoint generated by the pinned MPL-2.0
implementation. It exercises the real upstream tensor layout, safe importer,
CPU inference, duration/alignment behavior, mel probes, training hooks, and a
native HiFi-GAN composition without claiming pretrained speech quality.
