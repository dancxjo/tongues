...
test('browser microphone capture crosses the shared bounded server input', () => {
    ...
    assert.match(studioSource, /vad_backend: vadBackend/);
    assert.match(studioSource, /segmentation,/);
    assert.match(studioSource, /message\.speech_probability/);
    assert.match(studioSource, /message\.type === 'segment_opened'/);
    ...
    assert.match(studioSource, /\/api\/language-routing\/capabilities/);
    assert.match(studioSource, /Recognition language routing/);
    assert.match(studioSource, /Auto-detect fails closed/);
