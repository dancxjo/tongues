...
mod tests {
    ...
    #[test]
    fn vocab_is_data_driven_not_broad_seeded() {
        let v = Vocab::build(&[], &[], &[]);
        assert_eq!(v.get_id("<lang:lat>"), UNK_ID);
        assert_eq!(v.get_id("θ"), UNK_ID);
        assert!(v.size() < 32);

        let words = vec!["<lang:lat> ā".to_string()];
        let phonemes = vec!["θ".to_string()];
        let v = Vocab::build(&words, &phonemes, &[]);
        assert_ne!(v.get_id("<lang:lat>"), UNK_ID);
        assert_ne!(v.get_id("ā"), UNK_ID);
        assert_ne!(v.get_id("θ"), UNK_ID);
    }
}
