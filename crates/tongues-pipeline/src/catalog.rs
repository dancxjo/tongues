    ...
    let mut kinds = vec![
        ...
        component_node(
            ...
            vec![
                port("audio", Input, AudioBuffer, One, false),
                port("transcript", Input, TranscriptCommitted, Optional, false),
                port("pronunciations", Input, UtterancePlan, Many, false),
                port("acoustic_posteriors", Input, AcousticPosteriors, One, false),
                port("timing_hints", Input, Artifact, Optional, false),
                stream_port("hypotheses", Output, AlignmentHypotheses, Many, true),
                stream_port("deltas", Output, AlignmentDeltas, Many, true),
                port("artifact", Output, Artifact, Many, false),
                port("error", Output, Error, Many, false),
            ],
            "phone_alignment",
