...
use crate::models::manifest::{
    DEFAULT_LLM_MODEL_ID, DEFAULT_VOICE_MODEL_ID, ModelAsset, ModelBundle, ModelKind,
...
pub fn selected_llm_model_path() -> Result<PathBuf> {
    if let Some(path) = std::env::var_os("PRONLEX_LLM_MODEL") {
        return Ok(PathBuf::from(path));
    }
    let bundle = selected_bundle()?;
    let asset = bundle_primary_asset(bundle)?;
    Ok(asset_path(&resolve_pronlex_home()?, asset))
}

pub fn selected_llm_projector_path() -> Result<Option<PathBuf>> {
    if let Some(path) = std::env::var_os("PRONLEX_LLM_MMPROJ") {
        return Ok(Some(PathBuf::from(path)));
    }
    if std::env::var_os("PRONLEX_LLM_MODEL").is_some() {
        return Ok(None);
    }
    ...
    Ok(Some(asset_path(&resolve_pronlex_home()?, asset)))
}
...
pub fn resolve_pronlex_home() -> Result<PathBuf> {
    if let Some(home) = std::env::var_os("PRONLEX_HOME") {
        let home = PathBuf::from(home);
        if home.as_os_str().is_empty() {
            bail!("PRONLEX_HOME is set but empty");
        }
        return Ok(home);
    }

    let base = dirs::data_local_dir().context("failed to resolve local data directory")?;
    Ok(base.join("pronlex"))
}

pub fn model_selection_path() -> Result<PathBuf> {
    Ok(resolve_pronlex_home()?.join("model-selection.json"))
}
