...
pub fn ensure_selected_llm_available() -> Result<PathBuf> {
    ...
    if std::env::var_os("PRONLEX_LLM_MODEL").is_some() {
        ...
        anyhow::bail!(
            "PRONLEX_LLM_MODEL points to a missing or empty file: {}",
            path.display()
        );
    }
...
pub fn ensure_selected_llm_projector_available() -> Result<Option<PathBuf>> {
    ...
    if std::env::var_os("PRONLEX_LLM_MMPROJ").is_some() {
        if is_non_empty_file(&path) {
            return Ok(Some(path));
        }
        anyhow::bail!(
            "PRONLEX_LLM_MMPROJ points to a missing or empty file: {}",
            path.display()
        );
    }
...
pub fn ensure_asr_whisper_model_available() -> Result<PathBuf> {
    if let Some(path) = std::env::var_os("PRONLEX_ASR_WHISPER_MODEL") {
        let path = PathBuf::from(path);
        if is_non_empty_file(&path) {
            return Ok(path);
        }
        anyhow::bail!(
            "PRONLEX_ASR_WHISPER_MODEL points to a missing or empty file: {}",
            path.display()
        );
    }
...
pub fn fetch_model(model: Option<&str>, force: bool) -> Result<PathBuf> {
    if let Some(model) = model {
        ...
        let primary = bundle_primary_asset(bundle)?;
        return Ok(asset_path(&resolve_pronlex_home()?, primary));
    }
