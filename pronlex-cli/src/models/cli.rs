...
use crate::models::selection::{
    asset_path, bundle_present, is_non_empty_file, model_selection_path, resolve_pronlex_home,
...
pub enum ModelsCommand {
    #[command(about = "Choose the active LLM or Piper voice model")]
    Menu,
    ...
    Status,
    #[command(about = "Select the active model")]
...
pub struct ModelsUseCommand {
    #[arg(default_value = "piper-ryan-medium")]
    model: String,
}
...
fn print_paths(model: Option<&str>) -> Result<()> {
    ...
    println!("{}={}", "pronlex_home".cyan(), home.display());
...
fn print_status() -> Result<()> {
    ...
    if missing {
        println!("{} just fetch", "fetch with:".dimmed());
    }
        ...
        .filter(|bundle| bundle.kind == ModelKind::Asr)
    {
        ...
        println!("{} {} ({})", state, bundle.display_name.bold(), bundle.id);
        if !bundle_present(bundle)? {
            println!("{} just fetch asr", "fetch with:".dimmed());
        }
    }
...
fn select_model(model: &str) -> Result<()> {
    ...
    match bundle.kind {
        ...
        _ => {
            anyhow::bail!(
                "`{model}` is not a selectable model; use `just fetch {}`",
                bundle.id
            );
        }
    }
